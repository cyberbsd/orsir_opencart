...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		foreach ($data['products'] as $product) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "', converted_raw_bean_weight = '" . $product['converted_raw_bean_weight'] . "', subtract_quantity = '" . $product['subtract_quantity'] . "'");
	...
	public function confirm($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			foreach ($order_product_query->rows as $order_product) {
			  ...
			  // subtract weight
			  if( $order_product['converted_raw_bean_weight'] > 0 )
			  {
			    $this->db->query("UPDATE " . DB_PREFIX . "product SET weight = (weight - " . $order_product['converted_raw_bean_weight'] . ") WHERE product_id = '" . (int)$order_product['product_id'] . "' AND subtract_weight = '1'");
			  }
			  // subtract quantity
			  else
			  {
			    $quantity_to_be_subtracted =  (int)$order_product['quantity'];

			    if( $order_product['subtract_quantity'] > 0 )
			    {
			      $quantity_to_be_subtracted = (int)$order_product['subtract_quantity'];
			    }
				  $this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = (quantity - " . $quantity_to_be_subtracted . ") WHERE product_id = '" . (int)$order_product['product_id'] . "' AND subtract = '1'");
			  }
